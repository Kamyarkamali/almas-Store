...
const PowerSection = () => {
  ...
  const data = datas.filter((item: any) => item.category === "پاور");
  return (
    <div className="w-full rounded-md shadow-lg mx-auto mt-[1rem] h-auto bg-[#FFFFFF] items-center relative px-4">
      <Swiper
        slidesPerView={5}
        spaceBetween={10}
        pagination={{
        ...
        {data.map((product) => (
          <SwiperSlide key={product.id}>
            <div className="bg-white p-8 mt-16 flex flex-col items-center relative group">
              ...
              {/* نام محصول */}
              <h3 className="mt-2 text-center text-[12px] lg:text-[12px]">
                {product.nameProduct}
              </h3>
      ...
      <div className="absolute flex items-center justify-between w-full top-0 left-0 space-x-2 p-2 border-b-[1px] z-10">
        <div className="p-3">
          <h3 className="text-[22px] font-bold">پاور کامپیوتر</h3>
        </div>
